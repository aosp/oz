...
static int soc_pcm_init_runtime_hw(struct snd_pcm_substream *substream)
{
	...
	runtime->hw.rate_min = max(rate_min, cpu_stream->rate_min);
	runtime->hw.rate_max = min(rate_max, cpu_stream->rate_max);
...
static int soc_pcm_close(struct snd_pcm_substream *substream)
{
	 ...
	 */
	for (i = 0; i < rtd->num_codecs; i++)
		snd_soc_dai_digital_mute(codecs[i].codec_dai, 1, substream->stream);
...
static int soc_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	for (i = 0; i < rtd->num_codecs; i++)
		snd_soc_dai_digital_mute(codecs[i].codec_dai, 0, substream->stream);
