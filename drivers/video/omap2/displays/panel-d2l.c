...
int d2l_write_register(struct omap_dss_device *dssdev, u16 reg, u32 value)
{
	...
	ret = dsi_vc_send_long(dssdev, TCH, dsi_buf.data_type,
...
{
#define DUMPREG(r) printk(KERN_INFO "%-35s : %08x\n", #r, d2l_i2c_read(r))
...
#if 0
/* Currently not used we can pull in if needed */
...
static int d2l_dcs_read1(struct omap_dss_device *dssdev, u8 dcs_cmd, u8 * data)
{
	int ret;
	u8 buf[1];

	ret = dsi_vc_dcs_read(dssdev, TCH, dcs_cmd, buf, 1);
	if (ret < 0)
		return ret;
	*data = buf[0];
	return 0;
...
static int d2l_dcs_write1(struct omap_dss_device *dssdev, u8 dcs_cmd, u8 param)
{
	u8 buf[2];
	buf[0] = dcs_cmd;
	buf[1] = param;
...
static int d2l_get_id(struct omap_dss_device *dssdev,
		      u8 *id1, u8 *id2, u8 *id3)
{
	int ret = 0;

	PRINT_DEBUG("%s:%s\n", __FILE__, __func__);
	ret = d2l_dcs_read1(dssdev, DCS_GET_ID1, id1);

	if (ret)
		return ret;

	ret = d2l_dcs_read1(dssdev, DCS_GET_ID2, id2);
	if (ret)
		return ret;

	ret = d2l_dcs_read1(dssdev, DCS_GET_ID3, id3);
	if (ret)
		return ret;
			       ...
			       u16 *xres, u16 *yres)
{
	...
	PRINT_DEBUG(KERN_INFO "%s:%s xres %d yres %d\n", __FILE__, __func__, *xres,
		    *yres);
}
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	if (d2d->enabled) {
		dsi_bus_lock(dssdev);
		ret = d2l_dcs_read1(dssdev, DCS_READ_NUM_ERRORS, &errors);
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	if (d2d->enabled) {
		dsi_bus_lock(dssdev);
		r = d2l_get_id(dssdev, &id1, &id2, &id3);
		dsi_bus_unlock(dssdev);
	} else {
		r = -ENODEV;
	}
			       ...
			       const char *buf, size_t count)
{
	...
	if (d2d->enabled) {
		dsi_bus_lock(dssdev);
		if (!d2d->cabc_broken)
			d2l_dcs_write1(dssdev, DCS_WRITE_CABC, i);
		dsi_bus_unlock(dssdev);
	}
	d2d->cabc_mode = i;
...
static int d2l_power_on(struct omap_dss_device *dssdev)
{
	...
	if (d2d->enabled != 1) {
		...
		/* This register setting is required only if
		   host wishes to perform DSI read transactions. */
		...
		d2l_write_register(dssdev, HTIM1, 0x00200006);
		d2l_write_register(dssdev, HTIM2, 0x011A0400);
		d2l_write_register(dssdev, VTIM1, 0x000F0008);
		d2l_write_register(dssdev, VTIM2, 0x000F0300);
		      ...
		      u16 x, u16 y, u16 w, u16 h)
{
	...
	/* r = d2l_set_update_window(dssdev, x, y, w, h);
	   if (r)
	   goto err; */
...
static struct omap_dss_driver d2l_driver = {
	...
	/* .wait_for_te    = d2l_wait_te, */
	/* .setup_update   = d2l_setup_update, */
