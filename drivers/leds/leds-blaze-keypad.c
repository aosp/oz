...
static void blaze_keypad_led_store(struct led_classdev *led_cdev,
...
static int blaze_keypad_led_probe(struct platform_device *pdev)
{
	int ret;
...
static int blaze_keypad_led_remove(struct platform_device *pdev)
{
...
static struct platform_driver blaze_keypad_led_driver = {
	.probe = blaze_keypad_led_probe,
	.remove = blaze_keypad_led_remove,
	.driver = {
		   .name = "keypad_led",
		   .owner = THIS_MODULE,
		   },
...
static int __init blaze_keypad_led_init(void)
{
	return platform_driver_register(&blaze_keypad_led_driver);
}

static void __exit blaze_keypad_led_exit(void)
{
	platform_driver_unregister(&blaze_keypad_led_driver);
}

module_init(blaze_keypad_led_init);
module_exit(blaze_keypad_led_exit);

MODULE_DESCRIPTION("Blaze Keypad Lighting driver");
